...
```bash
docker build --build-arg REACT_APP_API_BASE_URL=http://localhost:3001/api -t react-contacts:0.1.0 -f Dockerfile .
```
...
```bash
docker run --env MONGODB_URI=mongodb://mongo:27017/react-contacts --link mongo -p 3001:3001 react-contacts:0.1.0
```
